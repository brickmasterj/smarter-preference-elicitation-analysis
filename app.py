...
def generate_graphs(weighing_method = 'RS'):
    ...
    try:
        ...
        attributes = format_data(csv_data, csv_labels, csv_type, weighing_method)
    except:
    ...
    weights_graph = [html.Div(children=[
        ...
        dcc.Dropdown(options=[{'label': 'RS - Rank Sum', 'value': 'RS'},
                     ...
                     value=weighing_method, id='weights-selector'),
...
app.layout = html.Div(children=generate_graphs(), id='graphs')

@app.callback(
    Output(component_id='graphs', component_property='children'),
    Input(component_id='weights-selector', component_property='value')
)
def update_layout(weighing_method):
    return generate_graphs(weighing_method)
